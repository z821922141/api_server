...
import { RequestMethod, RouterConfig, Handler, RunOptions } from "./types.ts";
/* 路由配置 */
const ROUTERS: Record<RequestMethod, Record<string, Array<(ctx: Context) => Response>>> = {
    "GET": {},
    "POST": {},
    "PUT": {},
    "DELETE": {},
...
export class Context {
     ...
     * 构造函数 
     */
    private constructor(request: Request, connInfo: ConnInfo, handler: Array<(ctx: Context) => Response>, handlerIndex: number) {
    ...
    static routerInit(routerConfig: RouterConfig, options: RunOptions) {
        ...
        for (const [key, value] of configList) {
            ...
            if (nowKey.endsWith("/_middleware.ts") && handler.MIDDLEWARE !== undefined) {
     ...
     * 参数 method - 路由请求方式  
     */
    static routerSet(path: string, fn: (ctx: Context) => Response, method: RequestMethod) {
